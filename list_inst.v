...
Proof.
  ...
  - rewrite -> if_true; auto.
    ...
    { rewrite /in_inset in H4.
      ...
      rewrite /is_Some.
      apply nzmap_elem_of_dom in H24.
      rewrite /is_Some in H24.
      destruct H24 as (? & H24).
      rewrite /inf in H24. rewrite E in H24. simpl in H24.
      eexists.
    ...
    + rewrite / out_map /inf /=.
      ...
      rewrite /inf in n.
      (*
      unfold lookup_total, nzmap_lookup_total in Hfl.
      simpl in Hfl.
      Check inset.
    ...
    (* set I0' := remap_out Ip tp new_node. *)
      ...
      eapply (contextualLeq_insert_node Ip I1 nullval new_node).
      set_solver. auto. rewrite H8. set_solver. rewrite H8. set_solver.
      apply nzmap_elem_of_dom_total in H4.  
  ...
  - (* tp <> nullval *)
    ...
    + rewrite /in_inset /inf lookup_insert. 
      ...
      apply nzmap_lookup_filter_Some.
      split; auto.
      rewrite Eqn /= in Hin. auto.
      rewrite /default /id.
      rewrite /inf in Hin. set_solver.
    + intros Hin.
      destruct Hin as (Hlt & Hin).
      rewrite /in_inset /out /out_map /inf /= lookup_insert in Hin. 
    ...
    intros I0 HI0p.
    apply (contextualLeq_insert_node Ip I1 tp new_node n').
    set_solver. auto. admit. admit. auto.
